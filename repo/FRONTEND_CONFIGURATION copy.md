...
**IMPORTANT**: For these endpoints, you MUST NOT send the Authorization header. If you send a Firebase token to a public endpoint, the backend will try to validate it, and if Firebase isn't configured locally, it will fail.

**Solution**: Only include the Authorization header for protected endpoints that require authentication.
...
// Add Firebase token to requests (only for protected endpoints)
...
const fetchEvents = async () => {
  // This is a public endpoint, so no Authorization header will be sent
  const response = await api.get('/events/');
  return response.data;
};

const fetchProfile = async () => {
  // This is a protected endpoint, so Authorization header will be sent
...
const getAuthHeaders = async (url, requireAuth = false) => {
  const headers = {
    'Content-Type': 'application/json',
  };
  
  // Only add Authorization header for protected endpoints
  if (requireAuth || !isPublicEndpoint(url)) {
    const auth = getAuth();
    const user = auth.currentUser;
...
const fetchEvents = async () => {
  const headers = await getAuthHeaders('/events/');
...
// Protected endpoint - auth token will be sent
const fetchProfile = async () => {
  const headers = await getAuthHeaders('/profile/', true);
...
2. ✅ **IMPORTANT**: Only include token in `Authorization: Bearer <token>` header for **protected** endpoints
3. ✅ **DO NOT** send Authorization header for public endpoints (`/events/`, `/providers/`, `/feed/`, etc.)
4. ✅ Handle token refresh when tokens expire (401 errors)
5. ✅ Use correct API base URL (dev vs production)
6. ✅ Ensure CORS is configured correctly
7. ✅ Public endpoints work without authentication (and must not receive auth tokens)
8. ✅ Protected endpoints require valid Firebase token

## Critical Rule

**Never send the Authorization header to public endpoints.** If you do, the backend will try to validate the token, and if Firebase isn't configured (like in local development), it will fail with a 401 error even though the endpoint should be publicly accessible.
